...
print(); print("=" * 100); print("F. FIGURES 1.5 AND 3.2 QUOTE (added in v5.0)"); print("=" * 100)
import re as _re
...
for _fn in os.listdir(_hist):
    _m = _re.match("^[ ]*([0-9]+)", _fn)
    if not _m: continue
    _t = open(os.path.join(_hist, _fn), encoding="latin-1", errors="replace").read()
    if _re.search("latent_trade_goods[^=]*=[^{]*{[^}]*coal", _t): _coal.add(int(_m.group(1)))
P("latent-coal provinces", len(_coal))
_own = [r for r in ROWS if r["pid"] in _coal]
P("of those owned and counted", len(_own))
_w2 = wealth.copy()
for _i, _r in enumerate(ROWS):
    if _r["pid"] in _coal: _w2[_i] = _r["tax"] + _r["gp"] * PRICES["coal"]
_rc = phi_w(1.5, _w2)
P("world wealth after repricing to coal", "%.1f -> %.1f" % (wealth.sum(), _w2.sum()))
P("Phi_w edge flips from the coal activation", "%d of %d" % (len(dws ^ set(_rc["directed"])) // 2, E))

_a15 = 1.5
_gi = GOODS.index("spices")
_S_eps, _, _, _, _, _ = build_sc(ALPHA, eps=1e-6)
def _spice_sinks(node, f):
    idx = np.array([k for k, r in enumerate(ROWS) if r["node"] == node])
    w = wealth.copy(); w[idx] *= f
    num = np.zeros(N); np.add.at(num, pn, w ** _a15); c = num / (w ** _a15).sum()
    d = orient(solve_phi(_S_eps[_gi] - c))
    od = collections.Counter(u for u, _ in d)
    return [i for i in range(N) if od[i] == 0], c
def _thresh(node):
    lo, hi = 1.0, 500.0
    for _ in range(45):
        mid = (lo + hi) / 2
        if NIDX[node] in _spice_sinks(node, mid)[0]: hi = mid
        else: lo = mid
    return hi
for _n in ("genua", "beijing", "hangzhou", "xian", "canton", "girin", "yumen", "chengdu", "lhasa"):
    _f = _thresh(_n); _sk, _c = _spice_sinks(_n, _f)
    P("spice-sink threshold, %s" % _n, "x%.3f  (%.1f%% of world spice demand)" % (_f, 100 * _c[NIDX[_n]]))
