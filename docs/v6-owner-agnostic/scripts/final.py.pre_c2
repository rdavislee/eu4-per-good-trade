...
"""
import numpy as np, collections, time, sys, os
...
print("V013 k values: %s" % collections.Counter(R[g]["info"]["k"] for _, g in GL))
...
for gi, g in GL:
    o = R[g]["order"]
    for i in range(N): Phi_ord[i] += V[gi] * o[i]
tie_edges = [(ORDER[u],ORDER[v]) for u,v in EDGES_UND if Phi_ord[u]==Phi_ord[v]]
print("V061 Phi_ord exact ties across an edge:", len(tie_edges), tie_edges)
agree = 0; tot = 0
for gi, g in GL:
    dset = set(R[g]["directed"])
    for u, v in EDGES_UND:
        gdir = (u,v) if (u,v) in dset else ((v,u) if (v,u) in dset else None)
        if gdir is None: continue
        tot += 1
        adir = (u,v) if Phi_ord[u] > Phi_ord[v] else (v,u)
        agree += (adir == gdir)
print("V062 Phi_ord agreement with own per-good graphs: %d/%d (%.1f%%)" % (agree, tot, 100*agree/tot))
...
for gi, g in GL:
    ...
    f, duals, res = mincost_flow(b + 0, np.zeros(N))  # b encodes supply-demand: pass s=b,c=0
    net = net_per_edge(f)
    flow_arc = {}; free = []
    for ei,(u,v) in enumerate(EDGES_UND):
