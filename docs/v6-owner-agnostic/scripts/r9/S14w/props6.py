...
if __name__ == "__main__":
    ...
    # C5: the scheduler-permutation figure section 1.1 quotes was computed in-tree only at width 2
    # before this loop (final.py's V035: sweep-only re-keying, 2 permutations per good); the cited
    # 145-permutation figure was run once in a throwaway script during the round-5 audit and then
    # cited, which is the defect Y250 names. Measured here so the citation is true because the
    # harness re-derived it.
    #
    # sweep_priority(pid=) re-keys ONLY the sweep -- Phase 1, the stall promotion and Phase 2 still
    # read the true index -- which is exactly the scope of the claim: that SCHEDULING does not decide
    # the orientation. It is not a relabelling test and must not be read as one (see relabel6.py).
    import numpy as _np
    from drain import sweep_priority as _sp, compile_dirs as _cd, phase0 as _p0, phase1 as _p1
    from drain import phase2 as _p2
    _rng = _np.random.default_rng(11)
    _changed = _trials = 0
    for _gi, _g in GL:
        _b = S[_g] - C[_g]
        _core, _beta, _Plog = _p0(_b)
        _S0, _info = _p1(_core, _beta)
        _fa, _free, _net, _cost = _p2(_core, _beta)
        _o1, _s1, _pr1, _fb1 = _sp(_core, _beta, _S0, _fa, _free, _net, "defasc_beta")
        _d1 = set(_cd(_core, _o1, _fa, _free, _Plog, _beta))
        for _k in range(5):
            _pid = {v: int(x) for v, x in zip(range(N), _rng.permutation(N))}
            _o2, _s2, _pr2, _fb2 = _sp(_core, _beta, _S0, _fa, _free, _net, "defasc_beta", pid=_pid)
            _d2 = set(_cd(_core, _o2, _fa, _free, _Plog, _beta))
            _trials += 1
            _changed += (_d1 != _d2)
    print("scheduler permutations: %d orientation changes over %d trials (%d goods x 5)"
          % (_changed, _trials, len(GL)))
