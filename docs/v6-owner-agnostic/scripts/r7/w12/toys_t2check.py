...
T4  V151: two equal-cost corridors; an infinitesimal demand perturbation swaps
    the LP support -> a link alternating support membership carries the FULL flow.
T5  V031 premise: disconnected graph with per-component imbalance -> LP infeasible.
"""
import numpy as np, collections, heapq
from scipy.optimize import linprog
from scipy.sparse import csr_matrix

def drain_generic(names, edges, b, verbose=False, wealth=None):
    N = len(names); E = len(edges)
    if wealth is None: wealth = [0.0] * N   # all tied -> the index decides, as 1.1 says
    ...
    while len(marked) < len(core):
        ...
        if found is None:
            ...
            if term:                                   # spec 1.1 Phase 3, promotion branch
                s_star = min(term, key=lambda v: (beta[v], v))
            else:                                      # spec 1.1 Phase 3, FALLBACK branch (v5.0)
                s_star = max(gated, key=lambda v: (wealth[v], -v))
    ...
    return set(sinks), claimed, directed, order, set(S), set(promotions), set(fallbacks)
...
b = [5.0, -3.0, 0.0, -2.0]
sk, cl, d, o, S0, PR, FB = drain_generic(names, edges, b, verbose=True)
...
sk, cl, d, o, S0, PR, FB = drain_generic(names, edges, b, verbose=True)
print("  T2 fallbacks:", sorted(names[i] for i in FB), " promotions:", sorted(names[i] for i in PR))

print()
print("="*90)
print("T3  the FALLBACK branch - a sink in neither {selected} nor {promoted}")
print("="*90)
# Triangle A,B,C with b = 0 everywhere.  No demander -> Phase 1 selects nothing; no flow -> every
# edge free; nothing ready -> the sweep stalls with no flow-terminal demander and the fallback
# branch fires.  Node wealth A=3 > B=2 > C=1.
names = ["A", "B", "C"]
edges = [(0,1), (0,2), (1,2)]
b = [0.0, 0.0, 0.0]
sk, cl, d, o, S0, PR, FB = drain_generic(names, edges, b, verbose=True, wealth=[3.0, 2.0, 1.0])
nm = lambda xs: sorted(names[i] for i in xs)
print("  FALLBACK promoted: %s   {selected} U {promoted} = %s" % (nm(FB), nm(S0 | PR) or "{}"))
print("  sink inside {selected} U {promoted}: %s   inside U {fallbacks}: %s"
      % (sk <= (S0 | PR), sk <= (S0 | PR | FB)))

print("="*90)
print("T4  V151 - equal-cost corridors: support membership alternates, flow is NOT small")
print("="*90)
...
print()
print("="*90)
print("T5  V031 premise - disconnected graph, per-component imbalance -> LP infeasible")
print("="*90)
