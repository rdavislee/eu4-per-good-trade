...
(spot-verified on 7 of 60 in the round-7 audit — C037, C038, C049, C101, C128, C130, C132 — none missing; no exhaustive re-check has been made). **v2.1** replaces the
...
- **Scan-invariance.** Ready-marking is a monotone closure, so the stall sequence and both
  ...
  shows it is that file verbatim, zero deletions, plus 27 added lines carrying the permutation
  loop. The loop was written for this citation — the figure had been quoted since v2 with
...
figure; the number of twelve runs landing inside that interval varies from 0 to 11 across
replicates — the variance is the refutation, and no counting triple is maintained in either
direction.)* That order of magnitude is reached with a generic LP; the all-29 figure is what a
...
Pattern scanning and function hooking, following the EU4dll precedent, which provides the attach scaffolding on this binary but nothing about trade structures. Ship a runtime-patching DLL, not a modified executable. The binary is frozen at build `835bfdf8` (SCM timestamp 2024-10-03, constant across `eu4_rev.txt` and all three crash dumps) — offsets found stay found for that binary, and a patch is a new binary: §2.3's re-measure-on-patch rule applied to offsets.
...
**Solver track** — the **defines parser first**: §2.3 makes every *define* the model reads a runtime read — `TAX_COEFF` is the one constant that is not, because it is in no shipped file that has been found (§2.3) — so the eligibility threshold, the propagation share, the off-home penalty, the merchant bonuses and the caravan terms are all downstream of it and cannot be written correctly before it exists. Then the full save parser (§2.2 item 2 — the reference reads the save only for the twenty rolled trade goods) and the `path`/`control` read that closes item 1's remaining gap; then the b-flow and sweep with their per-tick assertions (reachability, conservation, acyclicity, determinism, 2-core sink containment in `{selected} ∪ {promoted} ∪ {fallbacks}`) and
...
**Memory track** — the §2.7 probe session: the fourteen items still open there (1–11 and 16–18) on one trace.
