...
(`drain-orientation.md`; regenerated for v5.0 by `v5measure.py`). Where a property is proved for
...
159 `Φ_w` edges** (`v5measure.py`). Coal's base price of 10.0 is the highest in vanilla, so this is
...
one, and DRAIN's three knobs at their defaults — demand-mass
...
1. **Declaration order** — emit in decreasing `Φ_w` marking order. This is the convention the
   ...
   same world can produce two different maps. One visible consequence: the node window renders
...
| Most goods, 1444 | Sinks are `{selected ∩ flow-terminal} ∪ promoted` (§1.1) — 1 to 7 per good; high-demand nodes are sinks at 14.5% in the top demand decile vs 6.9% in the bottom (a barbell: LP branch ends land in poor pockets) |
...
drifting 0.012%). Any comparison against unmodded numbers needs a tolerance and a null run |
...
— 296.0, 299.2 and 266.5 against `english_channel`'s 316.6 — and none of them is a sink — draws more edges in than it sends out as a net demander even though flow passes through),
...
**Open in the v5.0 wealth model.** One question, and it is a question rather than a number — §1.3
...
- **The sink-count-span option.** A measured calibration exists that makes sink counts track price
  — span exactly 1..5, spearman(price, sinks) = −0.20: α unclamped at exponent 2 (cloves α = 16),
  ...
  individually carrying <0.03% of world supply — up to about **0.18%** of a good's mass in total,
  not <0.03% — and drops **cloves** to 99.97% reach, and it is one-snapshot
...
are sinks against 14.5%) and fails on delivery: it is monotone (§3.2), so demand must rise along
...
sinks per good against DRAIN's 1–7. *v2 said it "wins every sink statistic"; it does not — it wins
