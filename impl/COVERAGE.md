...
| E2 ★ treasury reconciliation | **PARTIAL** | instrumented at the exact point (pass 10 wrapped at `0xB4BF44`, sampled inside the same monthly update). Worst discrepancy **2.17 ducats**, down from 2438 when sampled late — so the earlier figure was almost entirely measurement lag. NOT called a pass: the engine's own routine is `AddDelayedIncome`, i.e. it queues income for the month boundary, so a same-tick sample structurally cannot observe the booking. Settling it needs the ledger object at `country+0x760` (category 2 = trade) |
| E3 ★ world total vs a null run | **OPEN** | needs two unmodded 12-month runs for the null spread |
| E4 ★ nothing NaNs, nothing leaks | **PASS** | 12/12 ticks CLEAN: no negative pool/outgoing/link/money, nothing non-finite, no runaway; world local/pool/links stable across the soak (7474→7456) |
