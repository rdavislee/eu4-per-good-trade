...
- Ship trade power propagates only where the country has a ship-propagation modifier, at a compounded rate the model reads as the propagation share multiplied by that modifier — the modifier and the share are each file-stated; their composition is stated by no file, string or observation, so the product is the model's derived reading rather than an engine fact.
...
1. **File values remembered from an older patch.** The 75% overseas autonomy floor is
   pre-Common-Sense. 1.37's regime floors are file-exposed: `COLONY_MIN_AUTONOMY = 50` in
