...
TIE_EPS = 1e-3                  # spec 2.3 Phase 2 tie-break strength; a hyperparameter
TIE_EPS2 = 1e-6                 # spec 2.3 second-order tie-break; a hyperparameter
...
# --------------------------------------------------- the Phase 2 tie-break (spec 2.3) --
# With unit arc costs the min-cost b-flow is degenerate: many routings reach the same minimum and the
# simplex returns whichever its pivot path reaches, which moves with node numbering. This cost breaks
# those ties inside the objective instead.
#
#     c(u,v) = 1 + TIE_EPS * (w[u] + w[v]) / 2
#
# It is SYMMETRIC in the arc, which matters twice. A directional cost `1 - eps*(w[v]-w[u])` is a
# potential difference, so its total over any flow meeting the same b is sum_n w[n]*b[n] -- constant,
# and unable to choose between routings at all. And because the cost reads node wealth only, it is
# invariant under relabelling, which is exactly the property unit costs lack.
NODEW = np.zeros(N)
for _r in ROWS:
    NODEW[NIDX[_r["node"]]] += _r["tax"] + _r["prod_income"]
_wn = (NODEW - NODEW.min()) / ((NODEW.max() - NODEW.min()) or 1.0)
# The first-order term is degenerate for SOME right-hand sides. It removes every alternative optimum
# on the aggregate b_w (40 zero-reduced-cost arcs outside the support -> 0) but leaves 2-6 on 18 of
# the 29 per-good b_g, because a different b builds a different spanning tree and exposes different
# arithmetic coincidences: a non-tree arc has zero reduced cost exactly when its own cost equals the
# sum of costs along the tree path between its endpoints. A structured cost invites such
# coincidences; a generic one does not. The second-order term below is that generic tie-break.
#
# It reads the UNORDERED wealth pair, so it stays symmetric in the arc and invariant under
# relabelling. Its form carries no design meaning -- it is tie-breaking and nothing else.
_a1 = np.array([_wn[u] for (u, v, _ei, _sg) in ARCS])
_a2 = np.array([_wn[v] for (u, v, _ei, _sg) in ARCS])
_gen = np.modf(np.minimum(_a1, _a2) * np.maximum(_a1, _a2) * 7919.0)[0]
TIE_COST = 1.0 + TIE_EPS * (_a1 + _a2) / 2.0 + TIE_EPS2 * _gen


# HiGHS stops when reduced costs are within its dual feasibility tolerance of zero. That default is
# 1e-7, and the margin by which the tie-break makes the optimum unique is as small as 3.8e-8 on some
# per-good solves -- INSIDE the default tolerance, so the solver was free to stop either side of the
# true optimum. Measured: under 6 permutations of the LP column order, `copper` and `paper` returned
# orientations differing on 12 and 8 edge-slots, with objectives differing by 7.7e-10 relative --
# six orders above float noise, so those were unequal-quality answers rather than tied optima.
# At 1e-10 (HiGHS's floor for these options) the flips go to 0 and the objective spread to 1.1e-15.
# This is a correctness requirement, not a performance knob: see spec 2.2 on multiplayer.
LP_OPTS = {"dual_feasibility_tolerance": 1e-10, "primal_feasibility_tolerance": 1e-10}


def mincost_flow(s, c, cost=None):
    """min sum(cost*f) s.t. inflow-outflow = c - s, f >= 0. Returns (arcflow, duals, res).

    cost=None means unit arc costs. The per-good operators in this file and in verify.py use unit
    costs; DRAIN's Phase 2 passes TIE_COST. See the TIE_COST note below."""
    b = c - s
    res = linprog(c=(np.ones(A) if cost is None else np.asarray(cost, dtype=float)),
                  A_eq=AEQ, b_eq=b, bounds=(0, None), method="highs", options=LP_OPTS)
    if not res.success:
