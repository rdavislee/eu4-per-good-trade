...
(spot-verified on 7 of 60 in the round-7 audit, none missing; no exhaustive re-check has been made). **v2.1** replaces the
...
sweep as documented apparatus is v5.0's alone; the one shipped sweep script is v4.0's
`audit_modifiers.py` — §2.3.)* On the 1444 start that apparatus was worth 105.30
...
no define, string or searched file names it — a probe-class fact under §3.16's own rule, recorded as
such)*. What trade range gates is **reach, not flow**: every string,
...
coefficient was in a file; v4.0's tree ships the one sweep script, `audit_modifiers.py`, which
walked past the block holding one of them, and v3.0 and v5.0 repeated the claim without shipping a
sweep.)*
...
it unset and setting it to 1e-7 give **identical results at every seed tried** — the flip count over
four permutations is seed-dependent (2, 8 or 6), so no single count is quoted, and the identity is
...
0.10 value-weighted), and nothing else measured moves. Sinks per good stay 2–8 mean 3.69, all 29 stay acyclic, `Φ_w`'s sinks are unchanged,
...
**Memory track** — the §2.7 probe session: the thirteen items still open there (1–11, 16 and 17) on one trace.
...
hop, so **17.1%** of world demand (unweighted per-good mean; 7.7% value-weighted) became unreachable and Genoa was crowned a cloves sink that
...
and a file with all 159 links declared backwards both loaded and played. The dumps show an unbounded same-module recursion on
the tested cycle. Acyclicity is enforced because the engine **demonstrably** did not survive the
...
1. **File values remembered from an older patch.** The 75% overseas autonomy floor is
   pre-Common-Sense; of 1.37's regime floors, `COLONY_MIN_AUTONOMY = 50` is the file-exposed one,
   and the 90/20/0 floors are not in `defines.lua` or any file that has been searched.
