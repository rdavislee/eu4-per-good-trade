...
# spec v5.0 §1.3: a modifier enters `wealth` iff it (a) depends only on the province's own
# attributes and on no country's state, and (b) modifies goods_produced, price or tax_value.
# Sources, all read from the install and all live at 1444 with no owner condition:
...
# Great-project province_modifiers accumulated to starting_tier, empty can_use_modifiers_trigger.
# NOTE: conditional on the Leviathan DLC - without it falun_copper_mine does not exist and
# province_triggered_modifiers/stora_kopparberget_modifier gives pid 8 trade_goods_size 5.0.
...
# add_permanent_province_modifier applied in the undated province-history block.
...
def province_table():
    ...
    for pid, s in PROV.items():
        ...
        gp = (gp + FLAT_GOODS.get(pid, 0.0)) * (1.0 + MON_GPMOD.get(pid, 0.0))
